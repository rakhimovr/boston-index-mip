...
data <- read_sav("spss_4_23_2021.sav")
lucid_data <-read.csv("lucid_response_4_23_2021.csv")
```

```{r, include = FALSE, echo = FALSE, results = 'asis', eval = FALSE}
# Incompletes match between Lucid and Alchemer
lucid <- read.csv("lucid_data_analysis_4_23_2021.csv")
...
- Sampled 1,123 adult residents of the Boston core-based statistical area (CBSA), quota matched on age, gender, and ethnicity and weighted by ZIP.
...
div(
  ...
  div(
    ...
    #class = "title",
    #h2("MIP Index: Boston"),
    "The survey was fielded from March 31 – April 21, 2021, drawing on the stratified sample of the Boston CBSA (n = 1,123)
    "
  ),
  # The actual table
  tt)
...
The table below presents average ranking in terms of carbon mitigation potential of MIP (highlighted) and low-impact behaviors assigned by participants. For example, on average, installing rooftop solar was assigned rank 4.61 out of 12.
...
##### Pearson correlations between demographic and pscyhographic predictors and behavior-related variables
...
tt3 = res.cor2 %>% 
  ...
  rename('Predictor' = term,
         ...
         'Beliefs that others should driven an EV' = ev_pnb,
         ...
         'Beliefs that others bougth offsets' = offset_ee,
...
ev_w <- reg %>% 
  lm(scale(ev_intent) ~ scale(ev_ee) + scale(ev_pnb) + scale(ev_ne) + scale(politics) + scale(income) + scale(education) + age_group + sex + ethnicity, data =.)

ev_d_w <- reg %>% 
  lm(scale(ev_intent) ~ scale(politics) + scale(income) + scale(education) + age_group + sex + ethnicity, data =.)
...
solar_w <- reg %>% 
  lm(scale(solar_intent) ~ scale(solar_ee) + scale(solar_pnb) + scale(solar_ne) + scale(politics) + scale(income) + scale(education) + age_group + sex + ethnicity, data =.)

solar_d_w <- reg %>%
    lm(scale(solar_intent) ~ scale(politics) + scale(income) + scale(education) + age_group + sex + ethnicity, data =.)
...
offset_w <- reg %>% 
  lm(scale(offset_intent) ~ scale(offset_ee) + scale(offset_pnb) + scale(offset_ne) + scale(politics) + scale(income) + scale(education) + age_group + sex + ethnicity, data =.)

offset_d_w <- reg %>% 
  lm(scale(offset_intent) ~ scale(politics) + scale(income) + scale(education) + age_group + sex + ethnicity, data =.)
...
recs_w <- reg %>% 
  lm(scale(recs_intent) ~ scale(recs_ee) + scale(recs_pnb) + scale(recs_ne) + scale(politics) + scale(income) + scale(education) + age_group + sex + ethnicity, data =.)

recs_d_w <- reg %>% 
  lm(scale(recs_intent) ~ scale(politics) + scale(income) + scale(education) + age_group + sex + ethnicity, data =.)
...
fly_less_w <- reg %>% 
  lm(scale(fly_intent) ~ scale(fly_ee) + scale(fly_pnb) + scale(fly_ne) + scale(politics) + scale(income) + scale(education) + age_group + sex + ethnicity, data =.)

fly_less_d_w <- reg %>% 
  lm(scale(fly_intent) ~ scale(politics) + scale(income) + scale(education) + age_group + sex + ethnicity, data =.)
...
pbd_w <- reg %>% 
  lm(scale(pbd_intent) ~ scale(pbd_ee) + scale(pbd_pnb) + scale(pbd_ne) + scale(politics) + scale(income) + scale(education) + age_group + sex + ethnicity, data =.)

pbd_d_w <- reg %>% 
  lm(scale(pbd_intent) ~ scale(politics) + scale(income) + scale(education) + age_group + sex + ethnicity, data =.)
...
food_waste_w <- reg %>% 
  lm(scale(foodwaste_intent) ~ scale(foodwaste_ee) + scale(foodwaste_pnb) + scale(foodwaste_ne) + scale(politics) + scale(income) + scale(education) + age_group + sex + ethnicity, data =.)

food_waste_d_w <- reg %>% 
  lm(scale(foodwaste_intent) ~ scale(politics) + scale(income) + scale(education) + age_group + sex + ethnicity, data =.)
...
##### Plots of standardized regression coefficients from linear models regressing intention to perform each behavior on each of the listed covariates

- Across the behaviors, the greatest predictors of intention to adopt the behaviors are the people's belief that other people are engaging in the behavior and people's beliefs that engaging in these behaviors is the right thing to do.
- Political orientation and demographic measures show inconsistent and largely insignificant effect.
- All models are adjusted for demographic controls for age, gender, ethnicity, household income, and education level.
...
##### Coefficients from linear models that included demographic and psychological variables

```{r, echo = FALSE, results = 'asis', message = FALSE, warning = FALSE, fig.width = 10, fig.height = 11}
# Use tibble dfs
models <- rbind(ev_w2, offset_w2, recs_w2, solar_w2, pbd_w2, food_waste_w2, fly_less_w2)
...
p <- {dwplot(models, 
    ...
    relabel_predictors(c("scale(ev_ee)" = "Others' behavior",                  
                         ...
                         "scale(income)" = "Household income",
                         "scale(education)" = "Education level",
                         "scale(politics)" = "Political orientation")) +
    xlab("Standardized coefficient estimate") + ylab("") + 
    theme_bw() +
    # Add colorblind-friendly palette & legend values = c("#000000","#D55E00","#CC79A7","#0072B2")
   ...
   theme(#axis.title = element_text(size = 16),
         ...
         legend.justification = c(0, 0), 
         # Legend bottom right position 
         legend.position = c(0.70, 0.000001),
         # Legend text size
         legend.text = element_text(size = 8, colour = "black"),
         #legend.title.align = .5,
         # Transparent legend text and glyph background
...
##### Coefficients from linear models that included demographic variables and political orientation only

```{r, echo = FALSE, results = 'asis', message = FALSE, warning = FALSE, fig.width = 7, fig.height = 9}
# Use tibble dfs
models <- rbind(ev_d_w, offset_d_w, recs_d_w, solar_d_w, pbd_d_w, food_waste_d_w, fly_less_d_w)
