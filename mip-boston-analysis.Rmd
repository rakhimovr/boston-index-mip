...
lucid_data <-read.csv("lucid_response.csv")
```
...
mutate(sex = case_when(sex == 10003 ~ "Male", TRUE ~ "Female"),
       ethnicity = case_when(ethnicity == 1 ~ "White", 
                             ...
                             ethnicity == 15 ~ "Other", 
                             TRUE ~ "NG"),
       ethnicity = case_when(Vrid == 394 ~ "White", Vrid == 1008 ~ "Asian", Vrid == 184 ~ "White", Vrid == 1505 ~ "Black", Vrid == 193 ~ "Black", TRUE ~ ethnicity)) %>% 
...
d <- merge(d, demo, by = c("ethnicity", "age_group", "sex"))
...
demo.dist <- data.frame(demo = demo$demo,
                       Freq = nrow(data) * demo$pop.demo.prop)
...
svy.rake <- rake(design = svy.unweighted,
                   sample.margins = list(~zip, ~demo),
                   population.margins = list(zip.dist, demo.dist))
...
data <- data %>% 
  select(wt, demo:var15)
```

### Interest in program participation (weighted by ZIP and age-sex-ethnicity)
...
violin_plot0 <- violin %>%
  ...
  ggplot(aes(x = reorder(as_factor(behavior), -value), y = value, fill = value, weight = wt)) +
...
div(
  ...
  #          Location: Boston CBSA | Period: Data collected between March 31, 2021 and April 21, 2021")
)
```
