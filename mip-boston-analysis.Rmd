...
p <- ggplot(violin_plot0, aes(x = behavior, y = value)) + 
  geom_violin()

p + geom_boxplot(width = 0.1, color = "grey") + stat_summary(fun.y=median, geom="point", size=2, color="red") + theme(axis.line = element_line(color = "grey"),
...
reactable(iris, columns = list(
  Sepal.Width = colDef(style = function(value) {
    normalized <- (value - min(iris$Sepal.Width)) / (max(iris$Sepal.Width) - min(iris$Sepal.Width))
