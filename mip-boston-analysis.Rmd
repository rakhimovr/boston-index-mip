...
pbd_w <- reg %>% 
  lm(scale(pbd_intent) ~ scale(pbd_ee) + scale(pbd_pnb) + scale(fly_ne) + scale(politics) + scale(income) + scale(education) + age_group + sex + ethnicity, data =.)
...
##### Plots of regression coefficients from linear models regressing intention to perform the behavior on each listed variables
...
- Political orientation shows inconsistent and largely insignificant effect.
- All models are adjusted for demographic controls for age, gender, ethnicity, household income, and education level.

```{r, echo = FALSE, results = 'asis', message = FALSE, warning = FALSE, fig.width = 9, fig.height = 5}
# Use tibble dfs
...
grid.draw(p)
ggsave("Fig2.tiff", units = "in", width = 9, height = 5, device='tiff', dpi = 300)
```
